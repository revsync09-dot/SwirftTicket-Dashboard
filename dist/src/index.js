import { ChannelType, Client, GatewayIntentBits, MessageFlags, Partials, PermissionFlagsBits, ModalBuilder, TextInputBuilder, TextInputStyle, ActionRowBuilder, } from 'discord.js';
import { config } from './config.js';
import { getGuildSettings, upsertGuildSettings } from './guildConfig.js';
import { claimTicket, closeTicket, createTicket, getTicketByChannel, getTicketByMessage, setTicketMessageId, getUserTicketHistorySummary, countRecentTicketsByCreator, updateTicketMetrics, reopenTicket, getGuildTicketStats, } from './tickets/repo.js';
...
client.on('guildCreate', async (guild) => {
    try {
        ...
        const channel = guild.channels.cache
            ...
            .find((c) => c?.viewable && c?.permissionsFor(guild.members.me)?.has(['SendMessages', 'ViewChannel']));
...
const SMART_REPLIES = [
    {
        keyword: 'banned',
        response: 'It looks like you mentioned a ban. Please include your username, the reason you believe you were banned, and any relevant context.',
    },
    {
        keyword: 'refund',
    ...
    {
        keyword: 'scam',
        response: 'If this is a scam report, include screenshots, user IDs involved, and any transaction links.',
    },
];
...
async function respondNotice(interaction, kind, title, body, ephemeral = true) {
    const payload = buildNotice(kind, title, body);
    const compFlag = MessageFlags.IsComponentsV2 ?? 1 << 15;
    const ephFlag = MessageFlags.Ephemeral ?? 1 << 6;
...
async function createSurface(interaction, parentId, staffRoleId) {
...
async function buildRenderContext(ticket, settings) {
...
async function createTicketWithReason(interaction, guildSettings, reason, category) {
    await interaction.deferReply({ flags: MessageFlags.Ephemeral ?? 1 << 6 });
    let surface;
    try {
        surface = await createSurface(interaction, guildSettings.ticket_parent_channel_id, guildSettings.staff_role_id);
    }
    catch (err) {
        await respondNotice(interaction, 'error', 'Ticket creation failed', err?.message ?? 'Unable to create ticket channel.');
        return;
    }
    ...
    await respondNotice(interaction, 'success', 'Ticket created', `Ticket ${ticket.id} created in ${surface.channel.toString()}.`, true);
}
async function handleTicketCreate(interaction, guildSettings) {
    const query = interaction.options.getString('reason', true);
    await createTicketWithReason(interaction, guildSettings, query);
}
async function rerender(interactionMessageId, channelId, mention, timezone) {
...
async function handleClaim(interaction, ticketId, staffRoleId, timezone) {
...
async function handleClose(interaction, ticketId, staffRoleId, timezone) {
...
async function handleTranscript(interaction, ticketId, timezone) {
    const ticket = await getTicketByMessage(interaction.message.id);
    if (!ticket) {
        await respondNotice(interaction, 'error', 'Ticket missing', 'This ticket could not be found.');
        return;
    ...
    try {
        if (interaction.isChatInputCommand()) {
            ...
            if (interaction.commandName === 'ticket') {
                ...
                if (sub === 'create') {
                    if (!settings) {
                        await respondNotice(interaction, 'error', 'Not configured', 'Ticket system not configured for this server. Run /ticket setup first.');
                        return;
                    }
                ...
                if (sub === 'setup') {
                    ...
                    const parentGuildId = parent.guildId ?? interaction.guildId;
                    ...
                    const parentCategoryId = parent.parentId;
                    if (!parentCategoryId) {
                        await respondNotice(interaction, 'error', 'Missing category', 'Selected channel must be inside a category.');
                        return;
                    }
                    ...
                    await respondNotice(interaction, 'success', 'Setup complete', `Configured tickets. Category: <#${parentCategoryId}> | Staff role: ${staffRole.toString()} | TZ: ${saved.timezone}`);
                }
            }
            ...
            if (interaction.commandName === 'mod') {
                    ...
                    if (actionType === 'WARN') {
                        ...
                        if (summary.warnings >= settings.warn_threshold) {
                            try {
                                ...
                                await respondNotice(interaction, 'error', 'Timeout failed', 'Warn logged but timeout could not be applied (check bot permissions).');
                                return;
                            }
                        }
                    }
                ...
                if (sub === 'config') {
                    ...
                    if (!current) {
                        await respondNotice(interaction, 'error', 'Not configured', 'Ticket system not configured. Run /ticket setup first.');
                        return;
                    }
                    ...
                    await respondNotice(interaction, 'success', 'Moderation configured', `Auto-timeout after ${threshold} warnings for ${minutes} minutes.`);
                }
            }
        }
            ...
            if (action === 'toggle' && idRaw) {
                ...
                const normalized = withDefaults(settings);
                if (idRaw === 'smart')
                    normalized.enable_smart_replies = !normalized.enable_smart_replies;
                if (idRaw === 'ai')
                    normalized.enable_ai_suggestions = !normalized.enable_ai_suggestions;
                if (idRaw === 'priority')
                    normalized.enable_auto_priority = !normalized.enable_auto_priority;
            ...
            if (action === 'claim')
                await handleClaim(interaction, ticketId, staffRoleId, timezone);
            if (action === 'close')
                await handleClose(interaction, ticketId, staffRoleId, timezone);
            if (action === 'transcript')
                await handleTranscript(interaction, ticketId, timezone);
        }
        ...
        if (interaction.isModalSubmit()) {
            ...
            if (scope === 'ticket' && kind === 'category' && action === 'create') {
                ...
                if (categories.length >= limit) {
                    await respondNotice(interaction, 'error', 'Slot limit reached', `You can only have ${limit} categories. Increase slots first.`);
                    return;
                }
        ...
        if (interaction.isRepliable()) {
            await respondNotice(interaction, 'error', 'Unexpected error', 'Something went wrong. Please try again or contact an admin.');
        }
    }
});
    ...
    try {
        ...
        if (aggressive.length && !isStaffMsg) {
            try {
                ...
                const alert = buildNotice('error', 'Safety Alert', `Message flagged for keywords: ${aggressive.slice(0, 4).join(', ')}`);
        ...
        if (isStaffMsg) {
            const update = { last_staff_message_at: now };
            if (!ticket.first_staff_response_at) {
                update.first_staff_response_at = now;
                update.first_response_ms = Math.max(0, new Date(now).getTime() - new Date(ticket.created_at).getTime());
            }
            if (ticket.last_user_message_at) {
                const delta = new Date(now).getTime() - new Date(ticket.last_user_message_at).getTime();
