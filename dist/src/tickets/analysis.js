...
export function analyzePriority(input) {
    ...
    const reasons = [];
    if (input.recentTickets >= 3)
        reasons.push('High ticket volume in 24h');
    if (input.repeatedReports >= 2)
        reasons.push('Repeated reports on same target');
    if (hits.length)
        reasons.push(`Flagged keywords: ${hits.slice(0, 4).join(', ')}`);
